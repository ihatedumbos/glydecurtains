...
export default function LineChart({ labels, data, color = '#925e41', height = 200 }: LineChartProps) {
  ...
  if (data.length === 0) {
    return (
      <div className="flex items-center justify-center text-[#8a97a0] text-sm" style={{ height }}>
        No data available
      </div>
    );
  }
    ...
    <div className="w-full">
      ...
      <div className="flex justify-between mt-2 px-2">
        {labels.filter((_, i) => i % Math.ceil(labels.length / 6) === 0 || i === labels.length - 1).map((label, index) => (
          <span key={index} className="text-xs text-[#8a97a0]">{label}</span>
        ))}
      </div>
    </div>
  );
