...
function AchievementCard({ achievement, index }: { achievement: Achievement; index: number }) {
  return (
    <motion.div
      ...
      className="group relative bg-[#ffffff] rounded-2xl p-8 shadow-md hover:shadow-xl transition-shadow duration-300 border border-[#2f3e46]/12"
    >
      {/* Icon */}
      ...
      {/* Year badge */}
      {achievement.year && (
        <div className="absolute top-4 right-4 text-xs font-medium text-[#8a97a0] bg-[#ffffff] px-2 py-1 rounded-full">
          {achievement.year}
        </div>
      )}
    </motion.div>
      ...
      <section className="relative py-20 px-4 md:px-8 bg-gradient-to-br from-primary/5 via-primary/10 to-transparent">
        <div className="max-w-7xl mx-auto text-center">
          <motion.h1
            ...
            className="text-3xl md:text-5xl font-bold text-[#2f3e46] mb-4"
          >
            Our Achievements
          </motion.h1>
          <motion.p
            ...
            className="text-lg text-[#5c6b73] max-w-2xl mx-auto"
          >
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="animate-pulse bg-[#ffffff] rounded-2xl p-8 shadow-md">
                  <div className="w-16 h-16 bg-[#f2f4f5] rounded-xl mb-5" />
                  <div className="h-8 bg-[#f2f4f5] rounded w-24 mb-2" />
                  <div className="h-5 bg-[#f2f4f5] rounded w-32 mb-2" />
                  <div className="h-4 bg-[#f2f4f5] rounded w-full" />
                </div>
              ))}
            </div>
          ) : achievements.length === 0 ? (
            <div className="text-center py-20 text-[#5c6b73]">
